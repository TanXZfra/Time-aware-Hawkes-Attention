...
import math
import numpy as np
from layers.Transformer_EncDec import Encoder, EncoderLayer, Decoder3, DecoderLayer2
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.d_model    = configs.new_d_model
        self.time_dim   = configs.time_dim
        self.n_heads    = configs.n_new_heads
        self.dropout    = configs.dropout

        # input embedding
        self.embed = DataEmbedding2(
            configs.enc_in, configs.new_d_model,
        ...
        # Hawkes attention
        self.hawkes_attn = HawkesAttention(
            new_d_model     = configs.new_d_model,
            n_new_heads     = configs.n_new_heads,
            time_dim        = configs.time_dim,
            tmlp_width      = configs.tmlp_width,
            tmlp_depth      = configs.tmlp_depth,
        ...
        # output projection
        mlp_layers = []
        in_dim = configs.new_d_model
        Act = nn.GELU if configs.activation.lower() == "gelu" else nn.ReLU
        for _ in range(configs.tmlp_depth):
            mlp_layers.append(nn.Linear(in_dim, configs.tmlp_width))
            mlp_layers.append(Act())
            mlp_layers.append(nn.Dropout(configs.dropout))
            in_dim = configs.tmlp_width
        mlp_layers.append(nn.Linear(in_dim, configs.enc_in))
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        x_mark_dec: [B, pred_len, time_dim]
        returns:    [B, pred_len, enc_in]
        """
        B = x_enc.size(0)


        hist_x = self.embed(x_enc, None)        # [B, seq_len, d_model]
        hist_t = x_mark_enc                     # [B, seq_len, time_dim]

        preds = []

        for i in range(self.pred_len):

            q_time = x_mark_dec[:, i:i+1, :]    # [B, 1, time_dim]

            out = self.hawkes_attn.forward(
                Q=hist_x[:, -1:, :],                 
                K=hist_x,
                V=hist_x,
                t_Q=q_time,
                t_K=hist_t,
                hawkes_self_attn_mask=False
            )  # [B,1,d_model]

            pred = self.output_mlp(out)         # [B,1,enc_in]
            preds.append(pred)

            new_emb = self.embed(pred, None)    # [B,1,d_model]
            hist_x = torch.cat([hist_x, new_emb], dim=1)
            hist_t = torch.cat([hist_t, q_time], dim=1)

        return torch.cat(preds, dim=1)           # [B, pred_len, enc_in]
